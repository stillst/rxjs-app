...
@Component({
  selector: 'app-take',
  templateUrl: './take.component.pug',
})
export class TakeComponent {
  source1$: Observable<number> = interval(1000);
  source1: Stream = getStreamObj(this.source1$, `interval(1000)`);
  result1: Stream = getStreamObj(this.source1$.pipe(take(5)), `interval(1000).pipe(findIndex(x => x % 2 === 0))`);
  ...
  result2: Stream = getStreamObj(this.source2$.pipe(take(5)), `
    from([{ name: 'Joe', age: 31 }, { name: 'Bob', age: 25 }, { name: 'Alic', age: 45 }]).pipe(take(5))
  `);

  source3$ = from([2, 30, 22, 5, 60, 1]);
  source3: Stream = getStreamObj(this.source3$, `from([2, 30, 22, 5, 60, 1])`);
  result3: Stream = getStreamObj(this.source3$.pipe(take(5)), `
    from([2, 30, 22, 5, 60, 1]).pipe(take(5))
  `);
}
