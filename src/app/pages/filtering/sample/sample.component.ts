...
export class SampleComponent {
  ...
  result1: Stream = getStreamObj(this.source1$.pipe(sample(this.source2$)), `interval(1000).pipe(sample(interval(2000)))`);
  result2: Stream = getStreamObj(this.source2$.pipe(sample(this.source1$)), `interval(2000).pipe(sample(interval(1000)))`);
}
