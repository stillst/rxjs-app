...
export class IgnoreElementsComponent {
  ...
  result1: Stream = getStreamObj(this.source1$.pipe(ignoreElements()), `interval(1000).pipe(ignoreElements())`);
  ...
  result2: Stream = getStreamObj(this.source2$.pipe(ignoreElements()), `
    from([{ name: 'Joe', age: 31 }, { name: 'Bob', age: 25 }, { name: 'Alic', age: 45 }]).pipe(ignoreElements())
  `);

  source3$ = from([2, 30, 22, 5, 60, 1]);
  source3: Stream = getStreamObj(this.source3$, `from([2, 30, 22, 5, 60, 1])`);
  result3: Stream = getStreamObj(this.source3$.pipe(ignoreElements()), `
