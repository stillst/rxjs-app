...
export class SkipWhileComponent {
  ...
  result1: Stream = getStreamObj(
    this.source1$.pipe(skipWhile(val => val < 5)),
    `interval(1000).pipe(skipWhile(val => val < 5))`
  );
  ...
  result2: Stream = getStreamObj(
    this.source2$.pipe(skipWhile(val => val.age > 30)),
    `from([{ name: 'Joe', age: 31 }, { name: 'Bob', age: 25 }, { name: 'Alic', age: 45 }])
  ...
  result3: Stream = getStreamObj(
    this.source3$.pipe(skipWhile(val => val < 50)),
