...
@Component({
  ...
  templateUrl: './skip-until.component.pug',
})
export class SkipUntilComponent {
  ...
  result1: Stream = getStreamObj(this.source1$.pipe(skipUntil(timer(6000))), `interval(1000).pipe(findIndex(x => x % 2 === 0))`);
  ...
  result2: Stream = getStreamObj(this.source2$.pipe(skipUntil(timer(6000))), `
    from([{ name: 'Joe', age: 31 }, { name: 'Bob', age: 25 }, { name: 'Alic', age: 45 }]).pipe(skipUntil(timer(6000)))
  `);

  source3$ = from([2, 30, 22, 5, 60, 1]);
  source3: Stream = getStreamObj(this.source3$, `from([2, 30, 22, 5, 60, 1])`);
  result3: Stream = getStreamObj(this.source3$.pipe(skipUntil(timer(6000))), `
    from([2, 30, 22, 5, 60, 1]).pipe(skipUntil(timer(6000)))
  `);
}
