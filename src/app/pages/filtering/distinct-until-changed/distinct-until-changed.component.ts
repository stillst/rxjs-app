...
export class DistinctUntilChangedComponent {
  ...
  result1: Stream = getStreamObj(this.source1$.pipe(distinctUntilChanged()), `interval(500).pipe(debounceTime(1000))`);
  ...
  source2: Stream = getStreamObj(
    this.source2$,
    `this.oneObj, this.twoObj, this.twoObj, { age: 7, name: 'Bar'}, { age: 7, name: 'Bar'}, this.oneObj, this.oneObj, this.twoObj,`,
  ...
  source3: Stream = getStreamObj(
    this.source3$,
    `of<{age: number, name: string}>({ age: 4, name: 'Foo'}, { age: 7, name: 'Bar'}, { age: 5, name: 'Foo'}, { age: 6, name: 'Foo'})`,
  );
